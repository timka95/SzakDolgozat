...
class APLoss (nn.Module):
    ...
    def compute_AP(self, x, label):
        N, M = x.shape
        x1 = 1-torch.sqrt(2.001 - 2*x)

        # quantize all predictions
        self.quantizer = self.quantizer.cuda() # cuda added by hichem
        q = self.quantizer(x1.unsqueeze(1))
        q1 = torch.min(q[:,:self.nq], q[:,self.nq:]).clamp(min=0) # N x Q x M

        nbs = q1.sum(dim=-1) # number of samples  N x Q = c
        rec = (q1 * label.view(N,1,M).float()).sum(dim=-1) # nb of correct samples = c+ N x Q
        prec = rec.cumsum(dim=-1) / (1e-16 + nbs.cumsum(dim=-1)) # precision
        rec2 = rec / rec.sum(dim=-1).unsqueeze(1) # norm in [0,1]

        ap = (prec * rec2).sum(dim=-1) # per-image AP
