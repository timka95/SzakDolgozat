...
def make_conv_block(in_channels, out_channels, kernel_size=3, stride=1, padding=0, dilation=1, groups=1, bias=True):
    #print(" ////////////////////////////// make_conv_block  ////////////////////////////// ")
    layers = []
...
class HT(nn.Module):
    ...
    def __init__(self, vote_index):
        ...
        self.vote_index = vote_index.view(self.r * self.c, self.h * self.w).to('cuda')  # Move to the same device as the model
    ...
    def forward(self, image):
        ...
        #print("Image reshaped size:", image.size())
        image = F.relu(image)
        #print("Image after ReLU size:", image.size())
        HT_map = image @ self.vote_index
        #print("HT_map size:", HT_map.size())
        HT_map = HT_map / self.norm
        HT_map = HT_map.view(batch, channel, -1).view(batch, channel, self.h, self.w)
        #print("HT_map reshaped size:", HT_map.size()
        return HT_map
...
class IHT(nn.Module):
    ...
    def __init__(self, vote_index):
        ...
        self.r, self.c, self.h, self.w = vote_index.size()
        self.vote_index = vote_index.view(self.r * self.c, self.h * self.w).t().to('cuda')
    ...
    def forward(self, input_HT):
        ...
        #print("-------------------INPUTHT, SELF VOTE------------------")
        #print(input_HT.size(), self.vote_index.size())
...
class HTIHT(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        hough_one=out
        out = self.iht(out)
        return out, hough_one


class CAT_HTIHT(nn.Module):
    #print(" ////////////////////////////// CLASS CAT_HTIHT  ////////////////////////////// ")

    def __init__(self, vote_index, inplanes, outplanes):
        #print(" ////////////////////////////// init CAT  ////////////////////////////// ")
        super(CAT_HTIHT, self).__init__()
        self.htiht = HTIHT(vote_index.to('cuda'), inplanes, outplanes)
        self.ht = HT(vote_index.to('cuda'))
        self.bn = nn.BatchNorm2d(inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.conv_cat = nn.Conv2d(inplanes+outplanes, inplanes, kernel_size=3, padding=1, bias=True)
    def forward(self, x):
        #print(" ////////////////////////////// forward CAT  ////////////////////////////// ")
        x = self.bn(x)
        x = self.relu(x)
        ...
        #print(" OUTFIRST", out_first)
        #y=self.ht(out_first)
        #print(" OUTFIRST_HT", y)
        #print()
        #out_second = self.conv_cat(torch.cat([hough_one,y], dim=1))
...
if __name__ == "__main__":
    ...
    inplanes =  256 # Replace 64 with the appropriate number based on your specific model architecture
    outplanes = 16  # Replace 64 with the appropriate number based on your specific model architecture
    ...
    model = HT.CAT_HTIHT(vote_index.to('cuda'), inplanes, outplanes)
